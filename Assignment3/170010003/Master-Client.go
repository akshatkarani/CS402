...
func askTime(ips string, conn *net.UDPConn, c chan string) {
	...
	var recv string
	logger.UnpackReceive("Time received", p, &recv, govec.GetDefaultLogOptions())
	p = p[:n]
	printErr(err)
	c <- string(recv)
}
...
func startSlave() {
	...
	for {
		...
		printErr(err)
		var msg string
		logger.UnpackReceive("Time request received", p, &msg, govec.GetDefaultLogOptions())
		localTime := time.Since(startTime).String()
		data := logger.PrepareSend("Local time sent", localTime, govec.GetDefaultLogOptions())
		...
		q = q[:n]
		var syncStr string
		logger.UnpackReceive("Synchronized time received", q, &syncStr, govec.GetDefaultLogOptions())
		syncTime, err := time.ParseDuration(syncStr)
		printErr(err)
		sync := time.Since(startTime) - syncTime
		startTime = startTime.Add(sync)
		logger.LogLocalEvent("Local time synchronized", govec.GetDefaultLogOptions())
